...
export const getBackendUrl = (): string => {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (hostname.includes('preview.emergentagent.com')) {
      return 'https://visa-wizard-1.preview.emergentagent.com';
    }
    
    // Production environment detection  
    if (hostname.includes('emergentagent.com') && !hostname.includes('preview')) {
      return 'https://visa-wizard-1.preview.emergentagent.com'; // Will be updated for production
    }
  }
  
  // Fallback to environment variable or preview URL
  return import.meta.env.VITE_BACKEND_URL || 'https://visa-wizard-1.preview.emergentagent.com';
};
