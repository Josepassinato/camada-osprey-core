...
class USCISPackageGenerator:
    ...
    def generate_official_form_pdf(self) -> bytes:
        ...
        field_style = ParagraphStyle(
            "FieldStyle", parent=styles["Normal"], fontSize=10, leading=14, fontName="Helvetica"
        )
        ...
        story.append(
            Paragraph(form_titles.get(self.form_code, "APPLICATION FORM"), styles["Normal"])
        )
        ...
        story.append(
            Paragraph(f"Form prepared on: {datetime.now().strftime('%B %d, %Y')}", styles["Normal"])
        )
    ...
    def generate_cover_letter(self) -> bytes:
        ...
        letter_style = ParagraphStyle(
            "LetterStyle", parent=styles["Normal"], fontSize=11, leading=16, alignment=TA_JUSTIFY
        )
        ...
        story.append(
            Paragraph(
                f"<b>Re: Application for Extension/Change of Status - Form {self.form_code}</b>",
                styles["Normal"],
            )
        )
        story.append(Spacer(1, 0.2 * inch))
        ...
        reason = self.user_story.get(
            "reasonForExtension",
            self.user_story.get(
                "user_story",
                "I respectfully request this change of status to pursue my goals in the United States.",
            ),
        )
    ...
    def generate_document_checklist(self) -> bytes:
        ...
        story.append(
            Paragraph(
                f"Applicant: {self.basic_data.get('firstName', '')} {self.basic_data.get('lastName', '')}",
                styles["Normal"],
            )
        )
        story.append(Spacer(1, 0.3 * inch))
    ...
    def generate_index_page(self) -> bytes:
        ...
        story.append(
            Paragraph(
                f"<b>Applicant:</b> {self.basic_data.get('firstName', '')} {self.basic_data.get('lastName', '')}",
                styles["Normal"],
            )
        )
        ...
        story.append(
            Paragraph(
                f"<b>Date Prepared:</b> {datetime.now().strftime('%B %d, %Y')}", styles["Normal"]
            )
        )
        ...
        index_data = [
            ...
            [
                "",
                "Form " + self.form_code,
                "Official USCIS application form (completed in English)",
            ],
            ["", "", ""],
        ...
        story.append(
            Paragraph("1. Review ALL documents carefully before mailing", styles["Normal"])
        )
        story.append(
            Paragraph("2. Sign Form " + self.form_code + " where indicated", styles["Normal"])
        )
        story.append(
            Paragraph("3. Make complete copies of everything for your records", styles["Normal"])
        )
        ...
        story.append(
            Paragraph(
                "<i>This package was prepared using Osprey Immigration Platform. All documents are organized in the proper order for USCIS submission.</i>",
                styles["Normal"],
            )
        )
    ...
    def generate_section_separator(
        self, section_number: int, section_title: str, description: str
    ) -> bytes:
    ...
    def generate_complete_package(self) -> bytes:
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
            ...
            section1 = self.generate_section_separator(
                1,
                "APPLICATION FORMS",
            ...
            zip_file.writestr(
                f"Section_1_APPLICATION_FORMS/02_Form_{self.form_code}_COMPLETED.pdf", official_form
            )

            # ============ SECTION 2: SUPPORTING DOCUMENTS ============
            section2 = self.generate_section_separator(
                2,
            ...
            for doc_name, doc_description in required_docs:
                ...
                user_doc = next(
                    (d for d in documents if doc_name.lower() in d.get("name", "").lower()), None
                )
            ...
            other_docs = [
                d
                for d in documents
                if not any(req[0].lower() in d.get("name", "").lower() for req in required_docs)
            ]
            ...
            section3 = self.generate_section_separator(
                3, "ADMINISTRATIVE", "This section contains checklists and mailing instructions"
            )
            ...
            zip_file.writestr(
                "Section_3_ADMINISTRATIVE/02_Mailing_Instructions.txt", instructions.encode("utf-8")
            )
    ...
    def generate_document_placeholder(self, doc_name: str, description: str) -> bytes:
        ...
        story.append(
            Paragraph(
                "Please insert this document here before mailing your application to USCIS.",
                styles["Normal"],
            )
        )
