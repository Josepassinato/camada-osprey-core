...
class DocumentDataExtractor:
    ...
    async def extract_and_validate(
        self, document_text: str, document_type: str, current_user_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not extracted_data:
                return {"success": False, "error": "Could not extract data from document"}
            ...
            should_auto_correct = self._should_auto_correct(
                document_type, discrepancies, extracted_data.get("confidence", 0)
            )
    ...
    def _extract_passport_data(self, document_text: str) -> Dict[str, Any]:
        ...
        extracted = {"confidence": 0.0, "fields": {}}
    ...
    def _find_discrepancies(
        self, extracted_data: Dict[str, Any], current_user_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
            ...
            for user_field in user_fields:
                if user_field in current_user_data:
                    ...
                    if extracted_field == "full_name":
                        is_different = self._compare_names(
                            extracted_value, current_value, current_user_data
                        )
                    else:
                        ...
                        discrepancies.append(
                            {
                                ...
                                "severity": (
                                    "high"
                                    if extracted_field in ["full_name", "date_of_birth"]
                                    else "medium"
                                ),
    ...
    def _compare_names(
        self, extracted_name: str, current_value: str, user_data: Dict[str, Any]
    ) -> bool:
        """
        ...
        if current_value:
            ...
            if current_clean in extracted_clean and len(extracted_clean) > len(current_clean):
                ...
                if len(parts_current) > 0 and all(
                    any(p1 in p2 or p2 in p1 for p2 in parts_extracted) for p1 in parts_current
                ):
                    # Nome do documento é mais completo - RETORNAR TRUE para indicar diferença
                    logger.info(
                        f"Document name is more complete: '{current_clean}' → '{extracted_clean}'"
                    )
        ...
        if first_name or last_name:
            ...
            if (
                full_current
                and full_current in extracted_clean
                and len(extracted_clean) > len(full_current)
            ):
                parts_current = [p for p in full_current.split() if p]
                parts_extracted = extracted_clean.split()

                if parts_current and all(
                    any(p1 in p2 or p2 in p1 for p2 in parts_extracted) for p1 in parts_current
                ):
                    # Nome do documento é mais completo - RETORNAR TRUE
                    logger.info(
                        f"Document name is more complete: '{full_current}' → '{extracted_clean}'"
                    )
                    return True  # É diferente e deve ser atualizado!
    ...
    def _should_auto_correct(
        self, document_type: str, discrepancies: List[Dict[str, Any]], confidence: float
    ) -> bool:
        ...
        if document_type == "passport":
            ...
            # Passaporte com confiança boa (>70%) e apenas discrepâncias críticas
                ...
                logger.info(
                    f"Auto-correcting critical fields based on passport with {confidence:.0%} confidence"
                )
                return True
        ...
        # Para certidão de nascimento, apenas com confiança muito alta
            ...
            logger.info(
                f"Auto-correcting based on birth certificate with {confidence:.0%} confidence"
            )
            return True
        ...
        # Para outros documentos, exigir confiança muito alta
            ...
            logger.info(
                f"Auto-correcting based on {document_type} with {confidence:.0%} confidence"
            )
            return True

        logger.info(
            f"Not auto-correcting: confidence={confidence:.0%}, discrepancies={len(discrepancies)}, type={document_type}"
        )
        return False

    def _generate_corrections(
        self, discrepancies: List[Dict[str, Any]], extracted_data: Dict[str, Any]
    ) -> Dict[str, Any]:
...
async def process_document_and_update_user(
    document_text: str, document_type: str, user_id: str, db
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not user:
            return {"extraction_successful": False, "error": "User not found"}

        # Extrair e validar dados
        extractor = DocumentDataExtractor()
        result = await extractor.extract_and_validate(document_text, document_type, user)
