...
class EnhancedDocumentRecognitionAgent:
    ...
    async def _analyze_document_visual(
        self, file_content: bytes, file_name: str, expected_type: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            # TODO: Refactor to use LLMClient properly
    ...
    async def _extract_document_data(
        self, file_content: bytes, document_type: str, visa_type: str
    ) -> Dict[str, Any]:
        """
    ...
    async def _analyze_document_quality(
        self, file_content: bytes, document_type: str
    ) -> Dict[str, Any]:
        """
    ...
    async def _analyze_visa_relevance(
        self,
        identified_type: str,
    ...
    async def _validate_document_ownership(
        self, extracted_data: Dict[str, Any], applicant_name: str, document_type: str
    ) -> Dict[str, Any]:
        """
    ...
    def _compile_final_result(
        ...
        if not type_matches:
            issues.append(
                f"Tipo de documento incorreto: esperado {expected_type}, identificado {document_analysis.get('identified_type', 'desconhecido')}"
            )
        ...
        if not relevance_analysis.get("document_required_for_visa", True):
            issues.append(
                f"Documento '{document_analysis.get('identified_type', expected_type)}' não é obrigatório para visto {visa_type}"
            )
    ...
    def _generate_recommendations(
        self, issues: List[str], relevance_analysis: Dict[str, Any]
    ) -> List[str]:
        """
        ...
        if issues:
            recommendations.append(
                "Revisar e corrigir os problemas identificados antes de submeter"
            )
