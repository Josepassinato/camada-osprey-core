...
class PassportValidator:
    ...
    def validate_passport_comprehensive(self, extracted_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        results["passport_number"] = self._validate_passport_number(
            extracted_data.get("passport_number", ""), extracted_data.get("nationality", "")
        )
        ...
        if "mrz_line1" in extracted_data and "mrz_line2" in extracted_data:
            mrz_result = self._validate_mrz(
                extracted_data["mrz_line1"], extracted_data["mrz_line2"]
            )
        ...
        return {
            ...
            "issues": issues,
            "uscis_acceptable": overall_confidence >= 90
            and all(
    ...
    def _validate_passport_number(self, number: str, nationality: str) -> ValidationResult:
        ...
        if not is_valid:
            issues.append(
                f'Passport number format invalid for {nationality or "unknown nationality"}'
            )
    ...
    def _cross_validate_mrz_visual(
        self, visual_data: Dict[str, Any], mrz_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if visual_passport and mrz_passport and visual_passport != mrz_passport:
            discrepancies.append(
                f"Passport number mismatch: Visual={visual_passport}, MRZ={mrz_passport}"
            )
        ...
        return {"has_discrepancies": len(discrepancies) > 0, "discrepancies": discrepancies}
...
class I797Validator:
    ...
    def validate_i797_comprehensive(self, extracted_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        results["notice_type"] = self._validate_notice_type(extracted_data.get("notice_type", ""))
        ...
        critical_valid = all(
            field in results and results[field].get("is_valid", False) for field in critical_fields
        )
...
class TranslationCertificateValidator:
    ...
    REQUIRED_PHRASES = ["complete and accurate", "competent to translate"]
    ...
    def validate_translation_certificate(self, document_text: str) -> Dict[str, Any]:
        ...
        has_date = bool(
            re.search(r"\d{1,2}[/-]\d{1,2}[/-]\d{2,4}|\w+\s+\d{1,2},\s+\d{4}", document_text)
        )
