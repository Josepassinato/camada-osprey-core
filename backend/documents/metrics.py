...
class DocumentAnalysisKPIs:
    ...
    def calculate_field_exact_match_rate(
        self, field_category: str, timeframe_days: int = 30
    ) -> float:
        """
    ...
    def calculate_processing_performance(self, timeframe_days: int = 30) -> Dict[str, float]:
        ...
        return {
            ...
            "within_target_percentage": sum(
                1 for t in processing_times if t <= self.targets["processing_time_target_ms"]
            )
        ...
        return {
            ...
            "kpis": {
                "classification_f1_score": {
                    ...
                    "status": (
                        "PASS"
                        if self.calculate_classification_f1(timeframe_days)
                        >= self.targets["classification_f1"]
                        else "FAIL"
                    ),
                },
                "identity_fields_exact_match": {
                    ...
                    "status": (
                        "PASS"
                        if self.calculate_field_exact_match_rate("identity", timeframe_days)
                        >= self.targets["identity_exact_match"]
                        else "FAIL"
                    ),
                },
                "dates_exact_match": {
                    ...
                    "status": (
                        "PASS"
                        if self.calculate_field_exact_match_rate("dates", timeframe_days)
                        >= self.targets["dates_exact_match"]
                        else "FAIL"
                    ),
                },
                "formatted_numbers_exact_match": {
                    "value": self.calculate_field_exact_match_rate(
                        "formatted_numbers", timeframe_days
                    ),
                    "target": self.targets["formatted_numbers_exact_match"],
                    "status": (
                        "PASS"
                        if self.calculate_field_exact_match_rate(
                            "formatted_numbers", timeframe_days
                        )
                        >= self.targets["formatted_numbers_exact_match"]
                        else "FAIL"
                    ),
                },
                "false_fail_rate": {
                    ...
                    "status": (
                        "PASS"
                        if self.calculate_false_fail_rate(timeframe_days)
                        <= self.targets["false_fail_rate"]
                        else "FAIL"
                    ),
                },
            },
...
class ConsistencyChecker:
    ...
    def check_name_consistency(self, documents_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if len(extracted_names) < 2:
            return {
                ...
                "message": "Need at least 2 documents with names",
            }
        ...
        return {"status": "consistent", "message": "All names are consistent across documents"}
    ...
    def check_date_consistency(self, documents_data: List[Dict[str, Any]]) -> Dict[str, Any]:
            ...
            for field_name, field_data in fields.items():
                if "date" in field_name.lower() and field_data.get("value"):
                    ...
                    dates_by_type[date_type].append(
                        {"date": field_data["value"], "document": doc_type, "field": field_name}
                    )
                    ...
                    for birth_entry in birth_dates:
                        for other_entry in other_dates:
                            if birth_entry["date"] >= other_entry["date"]:
                                issues.append(
                                    f"Birth date ({birth_entry['date']}) is after {other_type} date ({other_entry['date']})"
                                )
            ...
            for issue_entry in dates_by_type["issue"]:
                for expiry_entry in dates_by_type["expiry"]:
                    if issue_entry["date"] >= expiry_entry["date"]:
                        issues.append(
                            f"Issue date ({issue_entry['date']}) is after expiry date ({expiry_entry['date']})"
                        )
        ...
        return {"status": "consistent", "message": "All dates are logically consistent"}
