    ...
    def _extract_fields(self, text: str, doc_type: str) -> Dict:
        ...
        if doc_type == 'passport':
            ...
            passport_match = re.search(
                r'Passport No[:\s]+([A-Z0-9]+)', text, re.IGNORECASE
            )
            if passport_match:
            ...
            surname_match = re.search(
                r'Surname[:\s]+([A-Z\s]+)', text, re.IGNORECASE
            )
            if surname_match:
            ...
            sevis_match = re.search(
                r'SEVIS ID[:\s]+([A-Z0-9]+)', text, re.IGNORECASE
            )
            if sevis_match:
            ...
            school_match = re.search(
                r'School[:\s]+([A-Za-z\s]+(?:University|College|Institute))',
                text,
                re.IGNORECASE
            )
            if school_match:
            ...
            # Extract I-94 number
            i94_match = re.search(
                r'I-94 Number[:\s]+([0-9]+)', text, re.IGNORECASE
            )
            if i94_match:
