...
class DocumentClassifier(BaseAgent):
    """
    Automated immigration document classification system
    """

    def __init__(self, llm_client: Optional[LLMClient] = None):
        super().__init__(llm_client=llm_client, agent_name="document_classifier")
    ...
    async def process(self, input_data: Dict) -> Dict:
        """
        Process document classification request
        ...
        if use_llm:
            return await self.classify_document_with_llm(text_content, filename, file_size)
        else:
            return self.classify_document(text_content, filename, file_size)
    ...
    def _initialize_classification_patterns(self) -> Dict[str, Dict]:
        ...
        return {
            "PASSPORT_ID_PAGE": {
                ...
                "weight": {"required": 0.6, "optional": 0.3, "negative": -0.4, "filename": 0.1},
            },
            "BIRTH_CERTIFICATE": {
                "required_patterns": [
                ...
                "negative_patterns": [r"passport|marriage|diploma|employment"],
                "file_patterns": [
                    r"birth.*\.(pdf|jpg|jpeg|png)",
                    r"cert.*nasc.*\.(pdf|jpg|jpeg|png)",
                    r"nascimento.*\.(pdf|jpg|jpeg|png)",
                ],
                "weight": {"required": 0.7, "optional": 0.2, "negative": -0.3, "filename": 0.1},
            },
            "MARRIAGE_CERT": {
                "required_patterns": [
                ...
                "negative_patterns": [r"birth|passport|employment|diploma"],
                "file_patterns": [
            ...
            "DEGREE_CERTIFICATE": {
                ...
                "negative_patterns": [r"passport|birth|marriage|employment"],
                "file_patterns": [
                    r"diploma.*\.(pdf|jpg|jpeg|png)",
                    r"degree.*\.(pdf|jpg|jpeg|png)",
                    r"university.*\.(pdf|jpg|jpeg|png)",
                ],
                "weight": {"required": 0.6, "optional": 0.3, "negative": -0.3, "filename": 0.1},
            },
            "EMPLOYMENT_OFFER_LETTER": {
                ...
                "negative_patterns": [r"passport|birth|marriage|degree"],
                "file_patterns": [
            ...
            "I797_NOTICE": {
                ...
                "negative_patterns": [r"passport|birth|marriage|degree|employment offer"],
                "file_patterns": [
                    r"i?797.*\.(pdf|jpg|jpeg|png)",
                    r"uscis.*\.(pdf|jpg|jpeg|png)",
                    r"receipt.*\.(pdf|jpg|jpeg|png)",
                ],
                "weight": {"required": 0.8, "optional": 0.1, "negative": -0.2, "filename": 0.1},
            },
            "I94_RECORD": {
                "required_patterns": [
                    r"i-?94|form i-94",
    ...
    def classify_document(
        self, text_content: str, filename: str = "", file_size: int = 0
    ) -> Dict[str, Any]:
        """
    ...
    def _calculate_pattern_score(self, text: str, filename: str, patterns: Dict) -> float:
        ...
        if required_patterns:
            required_matches = sum(
                1 for pattern in required_patterns if re.search(pattern, text_lower, re.IGNORECASE)
            )
        ...
        if optional_patterns:
            optional_matches = sum(
                1 for pattern in optional_patterns if re.search(pattern, text_lower, re.IGNORECASE)
            )
        ...
        if negative_patterns:
            negative_matches = sum(
                1 for pattern in negative_patterns if re.search(pattern, text_lower, re.IGNORECASE)
            )
        ...
        if file_patterns and filename:
            file_matches = sum(
                1 for pattern in file_patterns if re.search(pattern, filename_lower, re.IGNORECASE)
            )
            if file_matches > 0:
    ...
    def _get_pattern_matches(
        self, text: str, filename: str, patterns: Dict
    ) -> Dict[str, List[str]]:
        """
        ...
        matches = {"required": [], "optional": [], "negative": [], "filename": []}
    ...
    def _combine_classification_results(
        self, signature_scores: Dict[str, float], pattern_results: Dict[str, Dict]
    ) -> Dict[str, Dict]:
        """
    ...
    def _determine_final_classification(self, combined_results: Dict[str, Dict]) -> Dict[str, Any]:
        ...
        if not combined_results:
            return {"document_type": "UNKNOWN", "confidence": 0.0, "status": "no_matches"}
    ...
    def _enrich_classification_result(
        self, classification: Dict[str, Any], text_content: str, filename: str, file_size: int
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_classification_suggestions(
        self, doc_type: str, confidence: float, status: str
    ) -> List[Dict[str, str]]:
        """
        ...
        if doc_type == "PASSPORT_ID_PAGE":
            suggestions.append(
                {"type": "info", "message": "Ensure MRZ (Machine Readable Zone) is clearly visible"}
            )
        elif doc_type == "I797_NOTICE":
            suggestions.append(
                {"type": "info", "message": "Verify receipt number format (3 letters + 10 digits)"}
            )
    ...
    def classify_multiple_documents(self, documents_data: List[Dict]) -> Dict[str, Any]:
        ...
        potential_duplicates = [
            doc_type for doc_type, count in doc_type_counts.items() if count > 1
        ]
        ...
        return {
            ...
            "statistics": {
                ...
                "success_rate": (
                    high_confidence_count / len(documents_data) if documents_data else 0
                ),
            },
        }
    ...
    async def classify_document_with_llm(
        ...
        """
        Classify document using LLM for enhanced accuracy
        ...
        try:
            ...
            rule_based = self.classify_document(text_content, filename, file_size)

            # If high confidence, return rule-based result
            if rule_based.get("confidence", 0) >= 0.85:
                rule_based["method"] = "rule_based"
                return rule_based

            # Use LLM for ambiguous cases
            document_types = list(self.classification_patterns.keys())

            messages = [
                ChatMessage(
                    role=MessageRole.SYSTEM,
                    content=(
                        "You are a document classification expert. Classify immigration "
                        "documents accurately based on their content."
                    ),
                ),
                ChatMessage(
                    ...
                    content=(
                        ...
                        f"Document content:\n{text_content[:2000]}\n\n"
                        f"Filename: {filename}\n\n"
                        f"Return a JSON object with: document_type, confidence (0-1), reasoning"
                    ),
                ),
            ]
            ...
            try:
                ...
                combined_confidence = (
                    rule_based.get("confidence", 0) * 0.3 + llm_result.get("confidence", 0) * 0.7
                )

                return {
                    "document_type": llm_result.get("document_type", rule_based["document_type"]),
                ...
                # LLM didn't return valid JSON, use rule-based
                rule_based["method"] = "rule_based_fallback"
                rule_based["llm_response"] = llm_response
                return rule_based
