...
# Passport Name Option Endpoints - Must be defined before router inclusion
@api_router.post("/case/{case_id}/use-passport-name")
async def use_passport_name_endpoint(
    case_id: str,
    request: dict
):
    """
    Atualiza os dados do caso com o nome do passaporte e reprocessa documento
    """
    try:
        passport_name = request.get("passport_name")
        document_filename = request.get("document_filename")
        
        if not passport_name:
            raise HTTPException(
                status_code=400,
                detail="Nome do passaporte é obrigatório"
            )
        
        logger.info(f"🔄 Updating case {case_id} with passport name: {passport_name}")
        ...
        # Atualizar os dados básicos com o nome do passaporte
        updates = {}
        
        # Atualizar form_data se existir
        if case_doc.get('form_data', {}).get('basic_info'):
            basic_info = case_doc['form_data']['basic_info'].copy()
            
            # Extrair primeiro e último nome do passaporte
            name_parts = passport_name.strip().split()
            if len(name_parts) >= 2:
                basic_info['firstName'] = name_parts[0]
                basic_info['lastName'] = ' '.join(name_parts[1:])
            else:
                basic_info['firstName'] = passport_name
                basic_info['lastName'] = ''
            
            basic_info['full_name'] = passport_name
            
            updates['form_data.basic_info'] = basic_info
        
        # Atualizar basic_data se existir  
        if case_doc.get('basic_data'):
            basic_data = case_doc['basic_data'].copy()
            
            name_parts = passport_name.strip().split()
            if len(name_parts) >= 2:
                basic_data['firstName'] = name_parts[0]
                basic_data['lastName'] = ' '.join(name_parts[1:])
            else:
                basic_data['firstName'] = passport_name
                basic_data['lastName'] = ''
                
            basic_data['full_name'] = passport_name
            
            updates['basic_data'] = basic_data
        
        # Adicionar metadata da alteração
        updates['passport_name_used'] = {
            "passport_name": passport_name,
            "updated_at": datetime.now(timezone.utc).isoformat(),
            "document_filename": document_filename,
            "reason": "user_chose_passport_name_over_registered_name"
        }
        ...
        if result.modified_count == 0:
            logger.warning(f"⚠️ No changes made to case {case_id}")
        
        logger.info(f"✅ Case {case_id} updated with passport name successfully")
        ...
        return {
            ...
            "message": f"Dados atualizados com o nome do passaporte: {passport_name}",
            "timestamp": datetime.now(timezone.utc).isoformat()
        }
        ...
        logger.error(f"❌ Error updating case with passport name: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Erro ao atualizar dados com nome do passaporte: {str(e)}"
        )

@api_router.post("/documents/reprocess-with-passport-name")
async def reprocess_document_with_passport_name(
    request: dict
):
    """
    Reprocessa um documento como aceito após atualização do nome
    """
    try:
        case_id = request.get("case_id")
        document_filename = request.get("document_filename") 
        ...
        if not all([case_id, document_filename, passport_name]):
            raise HTTPException(
                status_code=400,
                detail="case_id, document_filename e passport_name são obrigatórios"
            )
        
        logger.info(f"🔄 Reprocessing document {document_filename} for case {case_id} with passport name")
        
        # Criar nova análise aceita
        updated_analysis = original_analysis.copy()
        updated_analysis.update({
            "valid": True,
            "decision": "accepted",
            "name_mismatch_resolved": True,
            "passport_name_used": passport_name,
            "issues_found": [],
            "dra_paula_assessment": f"✅ DOCUMENTO ACEITO: Nome atualizado para '{passport_name}' conforme passaporte. Documento validado e aceito.",
            "updated_at": datetime.now(timezone.utc).isoformat(),
            "reprocessing_reason": "user_chose_passport_name"
        })
        
        # Se havia issues de nome, remover
        if "issues_found" in updated_analysis:
            issues = updated_analysis["issues_found"]
            updated_analysis["issues_found"] = [
                issue for issue in issues 
                if not any(keyword in str(issue).lower() for keyword in ['nome', 'name', 'corresponde', 'correspond'])
            ]
        
        logger.info(f"✅ Document reprocessed as accepted for case {case_id}")
        ...
        return {
            ...
            "message": "Documento reprocessado e aceito com nome do passaporte",
            "timestamp": datetime.now(timezone.utc).isoformat()
        }
