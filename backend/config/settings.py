...
class Settings(BaseSettings):
    ...
    mongodb_db: str = Field(default="test_database", description="MongoDB database name")
    ...
    log_pretty: bool = Field(
        default=False, description="Enable pretty-printed logs (for development)"
    )
    log_format: str = Field(default="json", description="Log format (json or text)")
    backup_dir: str = Field(default="./backups", description="Directory for database backups")

    # Sentry Configuration (Optional)
    sentry_dsn: Optional[str] = Field(default=None, description="Sentry DSN for error tracking")
    sentry_environment: str = Field(default="development", description="Sentry environment name")
    sentry_service_name: str = Field(default="backend", description="Service name for Sentry")
    sentry_server_name: Optional[str] = Field(default=None, description="Server name for Sentry")
    sentry_tags: str = Field(
        default="service=backend,component=api",
        description="Comma-separated key=value tags for Sentry",
    )
    sentry_release: Optional[str] = Field(default=None, description="Release version for Sentry")
    sentry_traces_sample_rate: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Sentry traces sample rate (0.0-1.0)"
    )
    sentry_profiles_sample_rate: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Sentry profiles sample rate (0.0-1.0)"
    )
    ...
    google_application_credentials: Optional[str] = Field(
        default=None, description="Path to Google Cloud credentials JSON file"
    )
    google_cloud_project_id: Optional[str] = Field(
        default=None, description="Google Cloud project ID"
    )
    google_cloud_location: str = Field(default="us", description="Google Cloud location/region")
    gemini_api_key: Optional[str] = Field(default=None, description="Google Gemini API key")

    # Stripe Configuration (Optional)
    stripe_secret_key: Optional[str] = Field(default=None, description="Stripe secret key")
    stripe_api_key: Optional[str] = Field(
        default=None, description="Stripe API key (legacy, use stripe_secret_key)"
    )
    ...
    resend_api_key: Optional[str] = Field(default=None, description="Resend email service API key")

    # Portkey Configuration
    portkey_api_key: Optional[str] = Field(
        default=None, description="Portkey API key for LLM routing and observability"
    )
    enable_portkey: bool = Field(
        default=False, description="Enable Portkey integration for LLM calls"
    )

    # Legacy LLM Keys (to be deprecated)
    emergent_llm_key: Optional[str] = Field(
        default=None, description="Legacy Emergent LLM key (deprecated, use Portkey)"
    )
    openai_api_key: Optional[str] = Field(
        default=None, description="OpenAI API key (legacy, use Portkey virtual keys)"
    )
