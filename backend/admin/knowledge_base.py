...
async def save_pdf_to_knowledge_base(
    file_content: bytes, filename: str, category: str, description: str, uploaded_by: str, db
) -> Dict:
    """
...
async def list_knowledge_base_documents(
    db, category: Optional[str] = None, limit: int = 100
) -> List[Dict]:
    """
    ...
    try:
        ...
        documents = (
            await db.knowledge_base_documents.find(query)
            .sort("uploaded_at", -1)
            .limit(limit)
...
async def get_knowledge_base_document(db, doc_id: str) -> Optional[Dict]:
    ...
    """
    try:
        document = await db.knowledge_base_documents.find_one({"id": doc_id, "status": "active"})
...
async def search_knowledge_base(db, query: str, category: Optional[str] = None) -> List[Dict]:
    ...
    try:
        ...
        # Buscar documentos
        documents = (
            await db.knowledge_base_documents.find(search_filter).limit(20).to_list(length=20)
        )
            ...
            results.append(
                {
                    ...
                    "snippet": snippet,
                    "uploaded_at": (
                        doc["uploaded_at"].isoformat() if doc.get("uploaded_at") else None
                    ),
                }
            )
