...
async def require_admin(current_user: Dict = Depends(get_current_user)):
    ...
    if user_role not in [UserRole.ADMIN, UserRole.SUPERADMIN]:
        raise HTTPException(
            status_code=403, detail="Admin access required. Your role: " + user_role
        )
...
async def require_superadmin(current_user: Dict = Depends(get_current_user)):
    ...
    if user_role != UserRole.SUPERADMIN:
        raise HTTPException(
            status_code=403, detail="Superadmin access required. Your role: " + user_role
        )
...
async def log_admin_action(
    ...
    try:
        ...
        logger.info(
            f"{status} AUDIT: {admin_user.get('email')} - {action} - {resource_type}:{resource_id}"
        )
...
class RateLimiter:
    ...
    async def check_rate_limit(
        self, key: str, max_requests: int = 100, window_seconds: int = 3600
    ) -> bool:
        """
        ...
        if key in self._requests:
            self._requests[key] = [
                (ts, count) for ts, count in self._requests[key] if ts > window_start
            ]
        else:
            self._requests[key] = []
...
async def check_admin_rate_limit(
    admin_user: Dict = Depends(require_admin), max_requests: int = 100
):
    """
    ...
    if not await rate_limiter.check_rate_limit(key, max_requests=max_requests):
        raise HTTPException(
            status_code=429, detail=f"Rate limit exceeded. Max {max_requests} requests per hour."
        )
