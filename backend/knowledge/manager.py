...
class KnowledgeBaseManager:
    ...
    async def get_documents_by_category(
        self, category: str, form_type: Optional[str] = None
    ) -> List[Dict]:
    ...
    async def get_document_by_id(self, document_id: str) -> Optional[Dict]:
        ...
        if document:
            ...
            await self.collection.update_one(
                {"document_id": document_id}, {"$inc": {"access_count": 1}}
            )
    ...
    async def list_all_documents(self, skip: int = 0, limit: int = 50) -> Dict:
        ...
        documents = (
            await self.collection.find({"status": "active"})
            .skip(skip)
            .limit(limit)
    ...
    async def update_document(self, document_id: str, updates: Dict) -> Dict:
        ...
        result = await self.collection.update_one({"document_id": document_id}, {"$set": updates})
