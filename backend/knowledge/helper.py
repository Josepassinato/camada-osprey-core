...
class AgentKnowledgeHelper:
    ...
    async def get_relevant_documents(
        self, form_type: str, category: Optional[str] = None, search_query: Optional[str] = None
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            if category:
                ...
                for cat in ["uscis_instructions", "formatting_guides", "document_requirements"]:
                    docs = await self.kb_manager.get_documents_by_category(
                        category=cat, form_type=form_type
                    )
                    documents.extend(docs)
            ...
            logger.info(
                ...
                extra={"form_type": form_type, "category": category, "count": len(documents)},
            )
    ...
    async def get_checklist(self, form_type: str) -> Optional[str]:
        ...
        try:
            ...
            docs = await self.kb_manager.get_documents_by_category(
                category="document_requirements", form_type=form_type
            )
    ...
    async def get_formatting_guide(self, form_type: str) -> Optional[str]:
        ...
        try:
            docs = await self.kb_manager.get_documents_by_category(
                category="formatting_guides", form_type=form_type
            )
    ...
    async def get_uscis_instructions(self, form_type: str) -> Optional[str]:
        ...
        try:
            docs = await self.kb_manager.get_documents_by_category(
                category="uscis_instructions", form_type=form_type
            )
    ...
    async def search_knowledge_base(
        self, query: str, form_type: Optional[str] = None
    ) -> List[Dict]:
        """
    ...
    async def get_context_for_agent(self, form_type: str, agent_role: str = "general") -> str:
        """
