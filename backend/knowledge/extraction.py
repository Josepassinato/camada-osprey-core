...
async def extract_assistant_knowledge(assistant_id: str = "asst_kkyn65SQFfkloH4SalOZfwwh"):
    ...
    try:
        ...
        # Save configuration
        config_filename = (
            f"dra_paula_assistant_config_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        if assistant.instructions:
            instructions_filename = (
                f"dra_paula_instructions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            )
        ...
        if hasattr(assistant, "file_ids") and assistant.file_ids:
            ...
            for i, file_id in enumerate(assistant.file_ids, 1):
                try:
                    ...
                    files_info.append({"file_id": file_id, "error": str(e)})

            # Save files manifest
            manifest_filename = (
                f"dra_paula_files_manifest_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
        ...
        summary_filename = (
            f"dra_paula_extraction_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        logger.info(
            f"✅ Instructions: {'Yes' if assistant.instructions else 'No'} ({len(assistant.instructions) if assistant.instructions else 0} chars)"
        )
