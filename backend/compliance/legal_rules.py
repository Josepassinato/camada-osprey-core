...
class ImmigrationLegalRules:
    ...
    def validate_f1_student_visa(form_data: Dict) -> Tuple[bool, List[str]]:
        ...
        if not english_proficiency or english_proficiency == "none":
            errors.append(
                "❌ OBRIGATÓRIO: Prova de proficiência em inglês (TOEFL, IELTS, Duolingo, etc.)"
            )
        ...
        if not has_i20 or has_i20 != "yes":
            errors.append(
                "❌ OBRIGATÓRIO: I-20 deve estar emitido pela instituição antes de prosseguir"
            )
        ...
        if wants_to_work == "yes":
            ...
            if work_type == "off_campus":
                errors.append(
                    "❌ TRABALHO OFF-CAMPUS NÃO PERMITIDO: Apenas trabalho on-campus (até 20h/semana) ou OPT após conclusão"
                )
            elif work_type == "on_campus":
                hours_per_week = form_data.get("work_hours_per_week", 0)
                if int(hours_per_week) > 20:
                    errors.append(
                        "❌ LIMITE EXCEDIDO: Trabalho on-campus limitado a 20 horas por semana"
                    )
        ...
        if plans_to_travel == "yes":
            warnings.append(
                "⚠️ ATENÇÃO CRÍTICA: Viajar para fora dos EUA durante o processo de visto F1 ANULA seu pedido. Não viaje!"
            )
        ...
        if current_status == "B2" and entry_date:
            try:
                ...
                if days_since_entry < 90:
                    errors.append(
                        f"❌ MUDANÇA B2→F1 NÃO PERMITIDA: Você deve esperar pelo menos 90 dias desde a entrada nos EUA. Faltam {90 - days_since_entry} dias."
                    )
            except:
    ...
    def validate_adjustment_of_status_marriage(form_data: Dict) -> Tuple[bool, List[str]]:
        ...
        if not currently_in_usa or currently_in_usa != "yes":
            errors.append(
                "❌ AJUSTE DE STATUS APENAS NOS EUA: Este processo é apenas para quem já está nos Estados Unidos. Se você está fora, deve fazer processo consular."
            )
        ...
        if not legal_entry or legal_entry != "yes":
            errors.append(
                "❌ ENTRADA LEGAL OBRIGATÓRIA: Você deve ter entrado legalmente nos EUA (ex: com visto B2, F1, etc.)"
            )
        ...
        if wants_travel_permit == "yes" and not form_data.get("has_i_131"):
            warnings.append(
                "💡 RECOMENDAÇÃO: Incluir formulário I-131 (Advance Parole para viagem)"
            )
    ...
    def validate_green_card_renewal(form_data: Dict) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            if days_until_expiration < 0:
                warnings.append(
                    f"⚠️ SEU GREEN CARD ESTÁ VENCIDO há {abs(days_until_expiration)} dias. Você pode continuar com a renovação, mas evite viajar internacionalmente."
                )

            # Mais de 6 meses para vencer
            elif days_until_expiration > 180:
                errors.append(
                    f"❌ MUITO CEDO: Você só pode renovar seu Green Card nos 6 meses anteriores ao vencimento. Seu Green Card vence em {days_until_expiration} dias. Volte em {days_until_expiration - 180} dias."
                )

            # Dentro do período permitido
            else:
                warnings.append(
                    f"✅ PERÍODO VÁLIDO: Seu Green Card vence em {days_until_expiration} dias. Você está dentro do período de renovação."
                )
    ...
    def validate_status_extension_b2(form_data: Dict) -> Tuple[bool, List[str]]:
        ...
        if entry_type == "ESTA":
            errors.append(
                "❌ ESTA NÃO PODE SER ESTENDIDO: O programa ESTA não permite extensão. Você deve sair dos EUA e retornar com novo ESTA ou visto."
            )
        ...
        if not entry_date:
            ...
            try:
                ...
                if days_since_entry < 90:
                    errors.append(
                        f"❌ MUITO CEDO PARA EXTENSÃO: Você deve esperar pelo menos 90 dias desde sua entrada nos EUA. Faltam {90 - days_since_entry} dias."
                    )
            except:
        ...
        if int(extension_months) > 6:
            warnings.append(
                "⚠️ ATENÇÃO: A extensão padrão é de 6 meses. Períodos maiores podem ser negados pelo USCIS."
            )
        ...
        if plans_to_travel == "yes":
            warnings.append(
                "🚨 ATENÇÃO CRÍTICA: Sair dos EUA durante o processo de extensão CANCELA automaticamente seu pedido. NÃO VIAJE!"
            )
    ...
    def validate_f1_reinstatement(form_data: Dict) -> Tuple[bool, List[str]]:
        ...
        if previous_status != "F1":
            errors.append(
                "❌ REINSTATEMENT APENAS PARA F1: Este processo é apenas para quem teve status F1 anteriormente"
            )
        ...
        if not termination_reason:
            errors.append(
                "❌ MOTIVO DO ENCERRAMENTO OBRIGATÓRIO: Informe por que seu status F1 foi encerrado"
            )
        ...
        if not has_new_i20 or has_new_i20 != "yes":
            errors.append(
                "❌ NOVO I-20 OBRIGATÓRIO: Você precisa de um novo I-20 da sua escola com indicação 'Reinstatement'"
            )
        ...
        if not i20_says_reinstatement or i20_says_reinstatement != "yes":
            errors.append(
                "❌ I-20 DEVE INDICAR REINSTATEMENT: Seu novo I-20 deve especificamente indicar que é para 'Reinstatement'"
            )
        ...
        if not has_i539 or has_i539 != "yes":
            errors.append(
                "❌ FORMULÁRIO I-539 OBRIGATÓRIO: Você deve preencher o formulário I-539 para Reinstatement"
            )

        # 5. Retorno às aulas
        warnings.append(
            "⚠️ IMPORTANTE: Você deve retornar às aulas IMEDIATAMENTE após receber o novo I-20. Não espere a aprovação do Reinstatement."
        )
    ...
    def validate_mandatory_documents(form_data: Dict, visa_type: str) -> Tuple[bool, List[str]]:
        ...
        if not passport_expiration:
            ...
            try:
                ...
                if exp_date < datetime.now():
                    warnings.append(
                        "⚠️ PASSAPORTE VENCIDO: Seu passaporte está vencido. Você pode continuar, mas isso pode afetar seu processo. Renove o quanto antes."
                    )
                elif (exp_date - datetime.now()).days < 180:
                    warnings.append(
                        "⚠️ PASSAPORTE PRÓXIMO DO VENCIMENTO: Seu passaporte vence em menos de 6 meses. Considere renová-lo."
                    )
            except:
        ...
        if bank_statement_translated != "yes":
            warnings.append(
                "⚠️ TRADUÇÃO OBRIGATÓRIA: Seu extrato bancário deve estar traduzido para inglês por tradutor juramentado"
            )
        ...
        if not has_brazil_ties or has_brazil_ties != "yes":
            errors.append(
                "❌ PROVAS DE VÍNCULO COM BRASIL OBRIGATÓRIAS: Contas, CNPS, declaração IR, propriedades, etc."
            )
        if ties_translated != "yes":
            warnings.append(
                "⚠️ TRADUÇÃO OBRIGATÓRIA: Documentos brasileiros devem estar traduzidos para inglês"
            )
        ...
        if has_dependents == "yes":
            ...
            for i in range(int(num_dependents)):
                ...
                if not dep_has_birth_cert or dep_has_birth_cert != "yes":
                    errors.append(
                        f"❌ CERTIDÃO DE NASCIMENTO DO DEPENDENTE {i+1} OBRIGATÓRIA (traduzida)"
                    )
        ...
        if not has_photos or has_photos != "yes" or int(num_photos) < required_photos:
            errors.append(
                f"❌ FOTOS OBRIGATÓRIAS: Você precisa de {required_photos} fotos padrão passaporte (2 por pessoa)"
            )
    ...
    @staticmethod
    def validate_travel_during_process(
        form_data: Dict, process_type: str
    ) -> Tuple[bool, List[str]]:
        """
        ...
        if plans_to_travel == "yes":
            if has_advance_parole == "yes":
                warnings.append(
                    "✅ ADVANCE PAROLE: Você possui Advance Parole e pode viajar. Leve o documento sempre com você!"
                )
            else:
                warnings.append(
                    f"🚨 PERIGO CRÍTICO: NÃO VIAJE durante o processo de {process_type}! Isso irá CANCELAR automaticamente seu pedido e você pode ter dificuldades para retornar aos EUA. Se precisar viajar, solicite Advance Parole (I-131) primeiro."
                )
