...
class InadmissibilityScreening:
    ...
    def get_screening_questions(self) -> List[Dict]:
        ...
        return [
            {"id": q["id"], "question": q["question"], "type": "yes_no"}
    ...
    def assess_risk(self, answers: Dict[str, str]) -> Dict[str, any]:
        ...
        for question in self.screening_questions:
            ...
            if answer == "yes":
                ...
                if question.get("follow_up"):
                    follow_ups.append({"question_id": q_id, "follow_up": question["follow_up"]})
        ...
        logger.info(
            f"📊 Inadmissibility screening completed: risk={overall_risk.value}, issues={len(issues)}"
        )

        return result

    def _generate_recommendations(
        self, risk: RiskLevel, inadmissibilities: List[InadmissibilityCategory], issues: List[Dict]
    ) -> List[str]:
        ...
        if risk == RiskLevel.LOW:
            recommendations.append(
                "✅ Seu caso parece direto. Você pode usar nossa plataforma com segurança."
            )
            ...
            recommendations.append(
                "✅ Se decidir prosseguir sozinho, revise tudo MUITO cuidadosamente."
            )
            recommendations.append(
                "📞 Considere pelo menos uma consulta com advogado antes de submeter."
            )

        elif risk == RiskLevel.HIGH:
            recommendations.append("🚨 Seu caso é COMPLEXO e requer atenção de advogado.")
            recommendations.append(
                "❌ NÃO RECOMENDAMOS usar nossa plataforma sem orientação legal."
            )
...
if __name__ == "__main__":
    ...
    test_answers_low = {q["id"]: "no" for q in screening.screening_questions}
    ...
    test_answers_high = {"visa_denial": "yes", "unlawful_presence": "yes", "criminal_record": "yes"}
