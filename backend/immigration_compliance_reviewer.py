...
class ImmigrationComplianceReviewer:
    ...
    def comprehensive_review(self, pdf_path: str, user_documents: Optional[Dict] = None) -> Dict:
        ...
        status = self._determine_final_status(
            compliance_score, critical_errors, major_errors
        )
        
        # Relatório final
        self._print_final_report(status, compliance_score, critical_errors, major_errors, minor_warnings)
    ...
    def _calculate_compliance_score(self, critical_errors: List, major_errors: List, warnings: List) -> int:
