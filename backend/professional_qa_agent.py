...
class ProfessionalQAAgent:
    ...
    def comprehensive_review(self, case_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if form_code not in self.knowledge_base:
            logger.warning(f"⚠️  Tipo de visto {form_code} não tem requisitos específicos cadastrados")
        ...
        critical_score, critical_issues = self._check_critical_criteria(
            case_data, requirements
        )
    ...
    def _check_professional_data(
        ...
        if form_code in ['O-1', 'EB-1A']:
            ...
            if not education or not any('Bachelor' in str(e) or 'Master' in str(e) or 'PhD' in str(e) for e in education):
    ...
    def _check_required_documents(
        ...
        unvalidated = [
            doc['filename'] 
            for doc in documents 
            if not doc.get('validated', False)
        ]
    ...
    def _generate_recommendations(
        ...
        for category, data in report['categories'].items():
            if data['score'] < 0.90:
                if category == 'personal_data':
                    recommendations.append("✅ Complete all personal information fields with accurate data")
                elif category == 'professional_data':
                    recommendations.append("✅ Enhance professional background with detailed achievements and qualifications")
                elif category == 'documents':
                    recommendations.append("✅ Upload all required supporting documents with proper validation")
        ...
        if report['approval']['approved']:
            ...
            recommendations.append(f"⏱️  Expected processing time: {requirements['processing_time']}")
        else:
            recommendations.append("❌ Application NOT approved - address all issues before submission")
            recommendations.append("⚠️  Submitting incomplete applications may result in rejection and loss of fees")
    ...
    def _log_review_result(self, report: Dict[str, Any]):
        ...
        logger.info(f"{status_icon} QA Review: {case_id} ({form_code}) - Score: {score:.1%} - {'APPROVED' if approved else 'REJECTED'}")
