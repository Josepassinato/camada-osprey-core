...
class VoiceAgent:
    ...
    async def _handle_snapshot(self, session_id: str, message: Dict) -> Dict:
        ...
        try:
            ...
            return {"type": "snapshot_received", "advice": asdict(advice), "session_id": session_id}
    ...
    async def _handle_voice_input(self, session_id: str, message: Dict) -> Dict:
        ...
        try:
            ...
            return {"type": "voice_response", "advice": asdict(advice), "intent": asdict(intent)}
    ...
    async def _get_section_guidance(
        self, section: Dict, fields: List[Dict], step_id: str
    ) -> AgentAdvice:
        ...
        try:
            ...
            if field_errors:
                ...
                say = (
                    f"A seção {section_label} está progredindo bem. Continue preenchendo os campos."
                )
    ...
    async def _validate_current_section(self, snapshot: Dict) -> AgentAdvice:
        ...
        try:
            ...
            return AgentAdvice(
                ...
                say=say,
                corrections=[
                    {"field": "vários", "reason": error, "how": "Verifique e corrija"}
                    for error in errors
                ],
                unresolved=warnings,
            )
...
"""
            ...
            # TODO: Refactor to use LLMClient from backend.llm.portkey_client
            ...
            response = await llm_client.chat_completion(
                messages=[
                    {
                        "role": "system",
                        "content": "Você é um assistente de conformidade que ajuda com preenchimento de formulários, mas nunca dá aconselhamento jurídico.",
                    },
            ...
            return AgentAdvice(
                ...
                say=ai_response,
                unresolved=[
                    "[Não verificado] Consulte um advogado para questões jurídicas específicas"
                ],
            )
