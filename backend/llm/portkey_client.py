...
class CircuitBreaker:
    ...
    def __init__(
        self, failure_threshold: int = 5, timeout_seconds: int = 60, half_open_attempts: int = 1
    ):
    ...
    async def chat_completion(
        ...
        **kwargs,
        ...
        for attempt in range(self.max_retries):
            try:
                logger.debug(
                    f"LLM request attempt {attempt + 1}/{self.max_retries} " f"to model {model}"
                )

                response = await asyncio.wait_for(
                    self._execute_chat_completion(request_params), timeout=self.timeout
                )
        ...
        try:
            ...
            if "not found" in str(e).lower() or "404" in str(e):
                raise PromptNotFoundError(
                    f"Prompt template '{prompt_id}' not found in Portkey", prompt_id=prompt_id
                )
            raise self._handle_exception(e, model or "unknown", 0)

    def _handle_exception(self, exception: Exception, model: str, attempt: int) -> LLMException:
