...
class QAFeedbackOrchestrator:
    ...
    async def orchestrate_qa_cycle(
        self, case_data: Dict[str, Any], qa_agent, db, max_iterations: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        if not self.learning_system:
            from learning.agent_learning import get_learning_system
        ...
        for iteration in range(1, max_iter + 1):
            ...
            if iteration > 1:
                ...
                if improvement < self.minimum_improvement:
                    ...
                    await self._save_final_result(
                        db, case_id, qa_report, iteration_history, "stalled"
                    )
            ...
            corrections_made = await self._execute_corrections(case_data, problems_by_agent, db)
        ...
        await self._save_final_result(
            db, case_id, final_qa, iteration_history, "max_iterations_reached"
        )
    ...
    def _classify_problems(self, qa_report: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for missing_item in qa_report.get("missing_items", []):
            ...
            if any(
                keyword in missing_item.lower()
                for keyword in ["document", "passport", "photo", "certificate"]
            ):
        ...
        for desc in all_descriptions:
            ...
            if any(
                ...
                for keyword in ["typo", "spelling", "grammar", "format", "language"]
            ):
    ...
    async def _execute_corrections(
        self, case_data: Dict[str, Any], problems_by_agent: Dict[str, List[Dict[str, Any]]], db
    ) -> List[Dict[str, str]]:
        """
        ...
        if problems_by_agent["document_analyzer"]:
            logger.info(
                f"📄 Encaminhando {len(problems_by_agent['document_analyzer'])} problemas para Document Analyzer"
            )

            action = await self._fix_document_issues(
                case_id, problems_by_agent["document_analyzer"], case_data, db
            )
        ...
        if problems_by_agent["form_filler"]:
            logger.info(
                f"📝 Encaminhando {len(problems_by_agent['form_filler'])} problemas para Form Filler"
            )

            action = await self._fix_form_issues(
                case_id, problems_by_agent["form_filler"], case_data, db
            )
            actions_taken.append(action)
        ...
        if problems_by_agent["translation_agent"]:
            logger.info(
                f"🔤 Encaminhando {len(problems_by_agent['translation_agent'])} problemas para Translation Agent"
            )

            action = await self._fix_language_issues(
                case_id, problems_by_agent["translation_agent"], case_data, db
            )
            actions_taken.append(action)
        ...
        if problems_by_agent["specialized_agent"]:
            logger.info(
                f"🎯 Encaminhando {len(problems_by_agent['specialized_agent'])} problemas para Specialized Agent"
            )

            action = await self._fix_specialized_issues(
                case_id, problems_by_agent["specialized_agent"], case_data, db
            )
            actions_taken.append(action)
    ...
    async def _fix_document_issues(
        self, case_id: str, problems: List[Dict[str, Any]], case_data: Dict[str, Any], db
    ) -> Dict[str, str]:
        """Corrige problemas relacionados a documentos"""
        try:
            # Importar agente
            from documents.analyzer import document_analyzer
            ...
            analysis_result = {"missing_critical_docs": [], "validation_issues": []}
            ...
            return {"agent": "document_analyzer", "status": "error", "error": str(e)}
        ...
        try:
            ...
            for idx, problem in enumerate(problems):
                await self._record_correction_lesson(
                    ...
                    correction={
                        "action": "form_filling",
                        "details": fixes_applied[idx] if idx < len(fixes_applied) else "No details",
                        "status": "completed",
                    },
            ...
            return {"agent": "form_filler", "status": "error", "error": str(e)}
        ...
        try:
            ...
            if isinstance(form_responses, dict):
                for key, value in form_responses.items():
                    if isinstance(value, str) and len(value) > 20:
                        texts_to_review.append({"field": key, "text": value})
            ...
            for idx, problem in enumerate(problems):
                await self._record_correction_lesson(
                    ...
                    problem=problem,
                    correction={
                        "action": "language_correction",
                        "details": corrections[idx] if idx < len(corrections) else {},
                        "status": "completed",
                    },
                    success=True,
                    form_code=case_data.get("form_code", "unknown"),
            ...
            return {"agent": "translation_agent", "status": "error", "error": str(e)}
    ...
    async def _fix_specialized_issues(
        ...
        try:
            ...
            return {"agent": "specialized_agent", "status": "error", "error": str(e)}
        ...
        for agent_name in [
            ...
            try:
                recommendations = await self.learning_system.get_preventive_recommendations(
                    agent_name, form_code, case_data
                )

                if recommendations:
                    logger.info(
                        f"💡 {len(recommendations)} recomendações preventivas " f"para {agent_name}"
                    )
    ...
    async def _record_correction_lesson(
        ...
        try:
            problem_with_context = {**problem, "form_code": form_code}
