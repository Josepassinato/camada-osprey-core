...
class ImmigrationExpert(BaseAgent):
    ...
    def __init__(
        ...
        logger.info(f"Initialized ImmigrationExpert (Dra. Paula B2C) with model {model}")

    async def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Process immigration-related requests

        Args:
            input_data: Dict containing:
                - action: "validate_form", "analyze_document", or "generate_advice"
        ...
        if action == "validate_form":
            return await self.validate_form_data(
            ...
            response = await self.generate_advice(
                question=input_data.get("question", ""), context=input_data.get("context")
            )
            return {"response": response}
        else:
            return {
                "error": f"Unknown action: {action}",
                "supported_actions": ["validate_form", "analyze_document", "generate_advice"],
            }

    async def _call_dra_paula(self, prompt: str) -> str:
        """
        Call Dra. Paula B2C assistant with enhanced context

        Args:
            prompt: User prompt to process

        Returns:
            Response from Dra. Paula
        """
        try:
            ...
            response = await self._call_llm(messages=messages, max_tokens=2000)
    ...
    async def validate_form_data(
        self, form_data: Dict[str, Any], visa_type: str, step_id: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            response = await self._call_dra_paula(prompt)
                ...
                "issues": [
                    {
                        "field": "system",
                        "severity": "high",
                        "message": "Erro temporário na análise da Dra. Paula.",
                    }
                ],
                "missing_info": [],
    ...
    async def analyze_document(
        ...
        Args:
            ...
            document_content: Content extracted from document
            visa_type: Type of visa being applied for
            user_data: User information for validation
        ...
        try:
            ...
            try:
                ...
                return {
                    ...
                    "critical_issues": [
                        {
                            "severity": "CRÍTICO",
                            "issue": "Erro na análise do documento",
                            "impact": "Não foi possível validar",
                        }
                    ],
            ...
            return {
                ...
                "critical_issues": [
                    {
                        "severity": "CRÍTICO",
                        "issue": "Erro técnico na análise",
                        "impact": "Documento não pôde ser validado",
                    }
                ],
                "data_inconsistencies": ["Erro no sistema"],
                "missing_elements": ["Análise completa"],
                "recommendations": [
                    "Tente fazer upload do documento novamente",
    ...
    async def generate_advice(self, question: str, context: Optional[Dict[str, Any]] = None) -> str:
        """
        ...
        Returns:
            Detailed immigration advice from Dra. Paula
        """
        try:
