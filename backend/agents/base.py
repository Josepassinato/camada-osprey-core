...
class BaseAgent(ABC):
        ...
        **kwargs,
        ...
        try:
            ...
            self._update_metrics(
                success=True, tokens=response.usage.total_tokens, latency_ms=latency_ms
            )
            ...
            logger.error(
                ...
                extra={"provider": e.provider, "model": e.model, "details": e.details},
            )
            self._update_metrics(success=False, error_type="provider_error")
            raise

        except LLMException as e:
            logger.error(f"[{self.agent_name}] LLM error: {e}", extra={"details": e.details})
            self._update_metrics(success=False, error_type="llm_error")
            raise

        except Exception as e:
            logger.error(f"[{self.agent_name}] Unexpected error in LLM call: {e}", exc_info=True)
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if metrics["successful_calls"] > 0:
            ...
            metrics["avg_latency_ms"] = metrics["total_latency_ms"] / metrics["successful_calls"]
        else:
