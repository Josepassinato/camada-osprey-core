...
class SpecializedAgentCoordinator:
    ...
    def __init__(
        ...
        self.agents = {
            "document_validator": DocumentValidationAgent(
                llm_client=self.llm_client,
                db=self.db
            ),
            "form_validator": FormValidationAgent(
                llm_client=self.llm_client,
                db=self.db
            ),
            "eligibility_analyst": EligibilityAnalysisAgent(
                llm_client=self.llm_client,
                db=self.db
            ),
            "compliance_checker": ComplianceCheckAgent(
                llm_client=self.llm_client,
                db=self.db
            ),
            "letter_writer": ImmigrationLetterWriterAgent(
                llm_client=self.llm_client,
                db=self.db
            ),
            "uscis_translator": USCISFormTranslatorAgent(
                llm_client=self.llm_client,
                db=self.db
            ),
            "triage": UrgencyTriageAgent(
                llm_client=self.llm_client,
                db=self.db
            )
        }
