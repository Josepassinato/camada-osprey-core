...
class DocumentValidationAgent(BaseAgent):
    ...
    async def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if validation_mode == "enhanced" and isinstance(document_data, bytes):
            ...
            return await self.validate_document(
                document_data=document_data, document_type=document_type, case_context=case_context
            )
    ...
    async def _get_knowledge_base_context(
        self, form_type: str, agent_role: str = "document_validation"
    ) -> str:
    ...
    async def validate_document_with_database(
        self, document_type: str, document_content: str, applicant_name: str, visa_type: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"agent": self.agent_name, "raw_response": response, "parse_error": True}

    async def validate_document(
        self, document_data: str, document_type: str, case_context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        visa_type = (
            case_context.get("visa_type") or case_context.get("form_code") if case_context else None
        )
        ...
        try:
            ...
            if quality_result["status"] == "fail":
                return self._create_fail_result(
                    "QUALITY_FAIL", quality_result["issues"], 0.0, quality_result["recommendations"]
                )
            ...
            final_confidence = (
                validation_result.get("overall_confidence", 0) if validation_result else 0
            )
            ...
            return {
                ...
                "document_type_identified": validation_result.get(
                    "document_type", expected_document_type
                ),
                "type_matches_expected": expected_document_type
                == validation_result.get("document_type", expected_document_type),
                "quality_acceptable": quality_result["status"] in ["ok", "alert"],
                "uscis_acceptable": uscis_acceptable,
                "issues": self._ensure_list(validation_result.get("issues", []))
                + self._ensure_list(quality_result.get("issues", [])),
                "recommendations": self._ensure_list(validation_result.get("recommendations", []))
                + self._ensure_list(quality_result.get("recommendations", [])),
                "detailed_analysis": {
            ...
            fallback_result = await self.validate_document(
                ...
                {"applicant_name": applicant_name, "visa_type": visa_type},
            )
    ...
    def _create_fail_result(
        self, reason: str, issues: List[str], confidence: float, recommendations: List[str]
    ) -> Dict[str, Any]:
    ...
    async def _extract_document_data_simulation(
        self, file_content: bytes, doc_type: str, file_name: str
    ) -> Dict[str, Any]:
        """
        Simulation of OCR data extraction
        In production would be replaced by real OCR (Tesseract + post-processing)
        """
        base_data = {"file_name": file_name, "file_size": len(file_content), "doc_type": doc_type}
