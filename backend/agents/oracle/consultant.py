...
# Update path to point to knowledge base at root level
DEFAULT_KB_PATH = (
    Path(__file__).resolve().parent.parent.parent.parent / "knowledge_base_documents.json"
)
...
class ImmigrationOracle:
    ...
    def __init__(self, kb_path: Optional[str] = None):
        ...
        logger.info(
            f"✅ Oráculo inicializado com {len(self.knowledge_base.get('documents', []))} documentos"
        )
    ...
    def get_required_documents(
        self, form_code: str, visa_category: Optional[str] = None
    ) -> List[str]:
        """
        ...
        return common_requirements.get(
            form_code,
            [
    ...
    def consult_general(self, query: str) -> Dict:
        ...
        for doc in self.knowledge_base.get("documents", []):
            ...
            if any(
                word in doc_text_lower or word in doc_name_lower for word in query_lower.split()
            ):
    ...
    def validate_document_checklist(self, form_code: str, submitted_docs: List[str]) -> Dict:
        ...
        return {
            ...
            "completion_percentage": (
                ((len(required) - len(missing)) / len(required) * 100) if required else 0
            ),
        }
    ...
    if query_type == "form_requirements":
        ...
        return {
            "required_documents": oracle.get_required_documents(
                kwargs.get("form_code", ""), kwargs.get("visa_category")
            )
        }
        ...
        return oracle.validate_document_checklist(
            kwargs.get("form_code", ""), kwargs.get("submitted_docs", [])
        )
