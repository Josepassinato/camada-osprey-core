...
def test_imports():
    ...
    try:
        ...
        assert hasattr(
            DraPaulaKnowledgeBase, "get_visa_specific_knowledge"
        ), "DraPaulaKnowledgeBase missing method"
        ...
        assert issubclass(
            DraPaulaGeminiAgent, BaseAgent
        ), "DraPaulaGeminiAgent doesn't inherit from BaseAgent"
        assert issubclass(
            HybridDraPaulaAgent, BaseAgent
        ), "HybridDraPaulaAgent doesn't inherit from BaseAgent"
        print("✅ Both agents inherit from BaseAgent")

        # Test knowledge base instance
        assert isinstance(
            dra_paula_knowledge, DraPaulaKnowledgeBase
        ), "dra_paula_knowledge is not an instance"
