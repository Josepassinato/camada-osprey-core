...
class HybridDraPaulaAgent(BaseAgent):
    ...
    async def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return await self.consult(
            question=question, visa_type=visa_type, context=context, agent_type=agent_type
        )

    async def consult(
        self, question: str, visa_type: str = None, context: str = None, agent_type: str = "general"
    ) -> Dict[str, Any]:
        """
...
"""
        ...
        # Build messages
        messages = self._build_messages(system_prompt=system_prompt, user_message=question)

        # Call OpenAI via BaseAgent's _call_llm (which uses Portkey)
        response_content = await self._call_llm(
            messages=messages, model="gpt-4o", temperature=0.7, max_tokens=2000
        )
    ...
    async def validate_document(
        self, document_type: str, document_data: Dict[str, Any], visa_type: str = None
    ) -> Dict[str, Any]:
        """Document validation with automatic fallback"""
        if self.gemini_agent:
            try:
                return await self.gemini_agent.validate_document(
                    document_type=document_type, document_data=document_data, visa_type=visa_type
                )
            except:
                pass
        ...
        question = (
            f"Validar documento {document_type}: {json.dumps(document_data, ensure_ascii=False)}"
        )
        return await self.consult(
            question=question, visa_type=visa_type, agent_type="document_validation"
        )

    async def check_eligibility(
        self, visa_type: str, applicant_profile: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Eligibility check with automatic fallback"""
        if self.gemini_agent:
            try:
                return await self.gemini_agent.check_eligibility(
                    visa_type=visa_type, applicant_profile=applicant_profile
                )
            except:
                pass
...
def get_dra_paula_agent(llm_client=None, prefer_gemini: bool = None) -> HybridDraPaulaAgent:
    ...
    if _hybrid_agent_instance is None:
        _hybrid_agent_instance = HybridDraPaulaAgent(
            llm_client=llm_client, prefer_gemini=prefer_gemini
        )
...
async def consult_dra_paula(question: str, visa_type: str = None, context: str = None) -> str:
    """
