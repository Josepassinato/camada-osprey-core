...
class IterativeLearningSystem:
        ...
        if review_result.get("critical_errors"):
            ...
            for i, error in enumerate(review_result["critical_errors"], 1):
                ...
                if "FORMULÁRIO OBRIGATÓRIO AUSENTE" in error:
                    ...
                    instructions.append(
                        f"   INSTRUÇÃO: Solicitar ao usuário renovação do passaporte. "
        ...
        if review_result.get("major_errors"):
            ...
            for i, error in enumerate(review_result["major_errors"], 1):
                ...
                if "Documento obrigatório ausente" in error:
                    ...
                    instructions.append(
                        f"   INSTRUÇÃO: Cada seção do pacote deve ter conteúdo ÚNICO e contextual. "
                    ...
                    instructions.append(
                        f"   INSTRUÇÃO: Adicione todas as seções obrigatórias do formulário. "
    ...
    def _generate_lca_field_instruction(self, field_name: str) -> str:
        ...
        return instructions_map.get(
            field_name, f"No LCA, preencha o campo: {field_name}. Este é um campo obrigatório."
        )
    ...
    def learn_from_rejection(self, review_result: Dict, instructions: List[str]):
        ...
        if len(self.lessons["correction_instructions"]) > 20:
            self.lessons["correction_instructions"] = self.lessons["correction_instructions"][-20:]
    ...
    def iterative_improvement_loop(
        self, initial_pdf_path: str, generator_function: callable
    ) -> Dict:
        """
    ...
    def print_learning_summary(self):
        ...
        if self.lessons["common_errors"]:
            ...
            sorted_errors = sorted(
                self.lessons["common_errors"].items(), key=lambda x: x[1], reverse=True
            )
