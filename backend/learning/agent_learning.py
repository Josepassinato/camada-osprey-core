...
class AgentLearningSystem:
    ...
    async def record_lesson(
        ...
        logger.info(
            f"📚 Lição registrada: {agent_name} - {problem.get('type')} " f"(Success: {success})"
        )

        return lesson

    async def get_relevant_lessons(
        self, agent_name: str, problem_type: str, form_code: Optional[str] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        logger.info(f"🔍 Encontradas {len(lessons)} lições para {agent_name} - {problem_type}")

        return lessons

    async def get_preventive_recommendations(
        self, agent_name: str, form_code: str, case_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for problem in common_problems:
            ...
            if risk_score > 0.5:  # 50% de probabilidade
                ...
                best_correction = await self._get_best_correction(
                    agent_name, problem["problem_type"]
                )
    ...
    async def _get_common_problems(
        self, agent_name: str, form_code: str, min_occurrences: int = 3
    ) -> List[Dict[str, Any]]:
        """Obtém problemas mais comuns para um agente e tipo de visto"""
        pipeline = [
            {"$match": {"agent_name": agent_name, "form_code": form_code}},
            {
                "$group": {
                    "_id": {"problem_type": "$problem.type", "description": "$problem.description"},
                    "count": {"$sum": 1},
    ...
    async def _get_best_correction(self, agent_name: str, problem_type: str) -> Dict[str, Any]:
        """Obtém a melhor correção conhecida para um tipo de problema"""
        # Buscar correções bem-sucedidas
        query = {"agent_name": agent_name, "problem.type": problem_type, "success": True}
    ...
    def _calculate_confidence(
        self, problem: Dict[str, Any], correction: Dict[str, Any], success: bool
    ) -> float:
    ...
    def _assess_risk(self, problem: Dict[str, Any], case_data: Dict[str, Any]) -> float:
    ...
    async def get_learning_statistics(
        ...
        return {
            ...
            "top_problems": [
                {"problem_type": p["_id"], "occurrences": p["count"]} for p in top_problems
            ],
            "agent_performance": [
                {
                    "agent": a["_id"],
    ...
    async def apply_learned_corrections(
        self, agent_name: str, case_data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for rec in recommendations:
            if rec["priority"] == "high" and rec["risk_score"] > 0.7:
                ...
                logger.info(f"🔧 Aplicando correção preventiva: {rec['problem_type']}")
        ...
        logger.info(f"✅ {len(corrections_applied)} correções preventivas aplicadas")
