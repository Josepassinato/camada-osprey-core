...
class CaseFinalizerComplete:
    ...
    def __init__(self):
        ...
        self.fees_kb = {
            "H-1B_basic": [
                ...
                {"code": "PREMIUM", "amount": 2805, "note": "Premium Processing (opcional)"},
                {
                    "code": "ACWIA",
                    "amount": 1500,
                    "note": "ACWIA Fee (primeira vez ou após 6+ anos)",
                },
                {
                    "code": "FRAUD_PREVENTION",
                    "amount": 500,
                    "note": "Fraud Prevention and Detection Fee",
                },
            ],
            "H-1B_extension": [
            ...
            "I-539_extension": [
                {
                    "code": "I-539",
                    "amount": 370,
                    "note": "Taxa base I-539 Extension/Change of Status",
                },
                {
                    "code": "BIOMETRICS",
                    "amount": 85,
                    "note": "Taxa de biometria (se solicitada pelo USCIS)",
                },
            ],
            "I-539_change": [
                {
            ...
            "I-485_employment": [
                ...
                {"code": "BIOMETRICS", "amount": 85, "note": "Taxa biometria"},
                {
                    "code": "I-765",
                    "amount": 0,
                    "note": "I-765 Work Authorization (incluído com I-485)",
                },
                {
                    "code": "I-131",
                    "amount": 0,
                    "note": "I-131 Travel Document (incluído com I-485)",
                },
            ],
            "I-485_family": [
                {"code": "I-485", "amount": 1140, "note": "Taxa I-485 Adjust Status"},
        ...
        self.addresses_kb = {
            ...
            "I-485_family": {
                "USPS": {
                    "name": "USCIS Chicago Lockbox",
                    "address_lines": ["USCIS", "P.O. Box 804625", "Chicago, IL 60680-4107"],
                }
            },
            "I-130_spouse": {
                "USPS": {
                    "name": "USCIS Chicago Lockbox",
                    "address_lines": ["USCIS", "P.O. Box 804616", "Chicago, IL 60680-4120"],
                }
            },
            "N-400_naturalization": {
                "USPS": {
                    "name": "USCIS Phoenix Lockbox",
                    "address_lines": ["USCIS", "P.O. Box 21251", "Phoenix, AZ 85036"],
                },
                "FedEx": {
                    "name": "USCIS Phoenix Lockbox",
                    "address_lines": [
        ...
        try:
            ...
            if case_data and AGENTS_AVAILABLE:
                ...
                if form_code in FORM_CODE_TO_VISA_TYPE:
                    logger.info(
                        f"🤖 Tentando gerar pacote com agente especializado para {form_code}"
                    )
                    ...
                    try:
                        ...
                        if agent_package_result.get("success"):
                            logger.info(
                                f"✅ Pacote gerado com sucesso pelo agente {agent_package_result['visa_type']}"
                            )
                            logger.info(
                                f"📊 QA Score: {agent_package_result.get('qa_report', {}).get('overall_score', 'N/A')}"
                            )
                        else:
            ...
            job_data = {
                ...
                "issues": [],
                "links": {},
                "estimated_timeline": self.supported_scenarios.get(scenario_key, {}).get(
                    "estimated_timeline", "Variable"
                ),
                "filing_location": self.supported_scenarios.get(scenario_key, {}).get(
                    "filing_location", "Check USCIS"
                ),
            ...
            logger.info(
                f"🚀 Case Finalizer Complete iniciado: job_id={job_id}, case_id={case_id}, scenario={scenario_key}"
            )
            ...
            if agent_package_result:
                ...
                logger.info(
                    f"✅ Case Finalizer Complete (Agent) concluído: job_id={job_id}, QA={qa_report.get('overall_score')}"
                )
            ...
            instructions_result = self._generate_instructions_complete(
                scenario_key, postage, language
            )
    ...
    def _audit_case_advanced(self, case_id: str, scenario_key: str) -> Dict[str, Any]:
        ...
        available_docs = [
            ...
            "tax_returns",
            "travel_records",
        ]  # Mock - comprehensive list for testing
    ...
    def _perform_scenario_specific_checks(
        self, scenario_key: str, available_docs: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        if scenario_key.startswith("H-1B"):
            ...
            if "diploma" in available_docs and "employment_letter" in available_docs:
                checks["recommendations"].append(
                    "Verificar se diploma está relacionado à posição oferecida"
                )
            ...
            checks["recommendations"].append(
                "Inclua I-765 e I-131 se desejar trabalhar/viajar durante processamento"
            )
    ...
    def _generate_audit_recommendations(
        self, scenario_key: str, missing_docs: List[str]
    ) -> List[str]:
        """
    ...
    def _generate_instructions_complete(
                    ...
                    "sections": [
                        {
                            ...
                            "content": [
                                "Siga as diretrizes padrão do USCIS para este tipo de petição"
                            ],
                        }
                    ],
                }
    ...
    def _generate_checklist_advanced(
        self, audit_result: Dict[str, Any], language: str
    ) -> Dict[str, Any]:
        """
        ...
        for check in quality_checks:
            checklist_items.append(
                {"item": f"🔍 {check}", "status": "review_needed", "note": "Revisar antes do envio"}
            )
    ...
    def _create_master_packet_real(
        self, case_id: str, audit_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "index_included": True,
                "file_size_mb": (
                    master_packet_path.stat().st_size / (1024 * 1024)
                    if master_packet_path.exists()
                    else 0
                ),
                "created_at": datetime.now(timezone.utc).isoformat(),
    ...
    def _generate_index_pdf(self, case_id: str, documents: List[Dict]) -> Path:
        ...
        content.append(
            Paragraph(
                "<i>This index is generated automatically. Please verify all documents are complete before submission.</i>",
                styles["Normal"],
            )
        )
    ...
    def _create_example_master_packet(self, output_path: Path, case_id: str, documents: List[Dict]):
        ...
        for document in documents:
            ...
            content.append(Spacer(1, 20))
            content.append(
                Paragraph("[This would be the actual document content]", styles["Normal"])
            )
            content.append(Spacer(1, 20))
            content.append(
                Paragraph(
                    "* This is a demonstration PDF. In production, actual scanned documents would be included here.",
                    styles["BodyText"],
                )
            )
            content.append(PageBreak())
    ...
    def get_job_status(self, job_id: str) -> Dict[str, Any]:
        ...
        return {"success": True, "job": {**job, "progress": progress}}
