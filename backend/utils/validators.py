...
def normalize_date(s: str, prefer_day_first: bool = True) -> str | None:
    ...
    tries = (
        (patterns_df + patterns_mdy + patterns_text)
        if prefer_day_first
...
_VALID_PREFIXES = {"EAC", "WAC", "LIN", "SRC", "MSC", "IOE", "NBC", "NSC", "TSC", "VSC", "YSC"}
...
def validate_passport_number_with_nationality(
    number: str, nationality: str
...
def validate_date_with_context(
    date_str: str, field_type: str = "general"
...
def enhance_field_validation(
    field_name: str, field_value: str, document_type: str, context: dict = None
) -> dict:
    """
    ...
    try:
        ...
        if "date" in field_name.lower():
            ...
            is_valid, confidence, normalized, error = validate_date_with_context(
        ...
        # Passport number
            ...
            is_valid, confidence, message = validate_passport_number_with_nationality(
        ...
        # USCIS Receipt Number
            ...
            field_name.lower() in ["receipt_number", "case_number"]
            and "i797" in document_type.lower()
        ):
            ...
            result.update(
                {
                    ...
                    "recommendations": (
                        [] if is_valid else ["Verify receipt number: 3 letters + 10 digits"]
                    ),
                }
            )
            ...
            result.update(
                {
                    ...
                    "recommendations": (
                        []
                        if is_valid
                        else ["Check SSN format: XXX-XX-XXXX with valid area/group/serial"]
                    ),
                }
            )
