...
class VisaAutoUpdater:
    def __init__(self, db: AsyncIOMotorDatabase):
        self.db = db
        # Initialize LLM client with Portkey (using Gemini for visa analysis)
    ...
    async def _scrape_uscis_processing_times(
    ...
    def _extract_processing_time(self, soup: BeautifulSoup, form_code: str) -> Dict[str, Any]:
        ...
        for element in time_elements:
            ...
            time_match = re.search(
                r"(\d+\.?\d*)\s*(?:to|-)?\s*(\d+\.?\d*)\s*(month|day|week)", text, re.IGNORECASE
            )
            if time_match:
        ...
        for table in tables:
            ...
            for row in rows:
                ...
                if len(cells) >= 2:
                    ...
                    if any(
                        cat in category.upper()
                        for cat in ["EB-1", "EB-2", "EB-3", "F1", "F2", "F3", "F4"]
                    ):
    ...
    async def _detect_changes_with_ai(
        ...
        for data in scraped_data:
            try:
                ...
                change_analysis = await self._analyze_changes_with_ai(
                    current_data, json.loads(data.content), data.form_code, data.data_type
                )
    ...
    async def _analyze_changes_with_ai(
        self, old_data: Dict, new_data: Dict, form_code: str, data_type: str
    ) -> Dict[str, Any]:
        ...
        try:
            # Create messages for LLMClient
            messages = [
                ChatMessage(
                    role=MessageRole.SYSTEM,
                    content="You are an AI assistant that analyzes visa information changes for USCIS, State Department, and Federal Register updates. Provide factual, educational information only. Always respond with valid JSON.",
                ),
                ChatMessage(role=MessageRole.USER, content=prompt),
            ]

            # Use LLMClient with Portkey
            response = await self.llm_client.chat_completion(
                messages=messages,
                model="gemini-1.5-pro",
                temperature=0.3,  # Lower temperature for more consistent JSON
                metadata={
                    "task": "visa_change_analysis",
                    "form_code": form_code,
                    "data_type": data_type,
                },
            )
            ...
            return {
                ...
                "description": f"Could not parse AI response: {e}",
    ...
    async def _get_current_visa_data(
        self, form_code: str, data_type: str
    ) -> Optional[Dict[str, Any]]:
        """Get current visa information from database"""
        try:
            doc = await self.db.visa_information.find_one(
                {"form_code": form_code, "data_type": data_type, "is_active": True}
            )
